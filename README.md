...
该项目本是进行`TCP服务器`进行数据传输然后`远程调用`，但是会用一个问题，进行TCP端口连接还要新开一个端口，远程和本地要有完全限定类名才能进行调用，要规定数据传输协议传递目标调用Class类型，只要本地有该Class就可以进行反射调用，这样就形成了一种强约束（我不是很喜欢），再因为要集成`Spring MVC`、简化数据解析和封装，底层复杂度，**最后决定实现HTTP请求的方式进行远程调用（请求）**，`【参考 OpenFeign、Mybatis底层源码（接口代理）】`。
